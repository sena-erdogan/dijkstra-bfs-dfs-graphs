...
public class MatrixGraph<E extends Comparable<E>>
    ...
    public MatrixGraph(int numV, boolean directed) {
        super(numV, directed);
        edges = (E[][]) new Object[numV][];
        if (!directed) {
            for (int i = 0; i != numV; ++i) {
                edges[i] = (E[]) new Object[numV];
    ...
    public void insert(Edge edge) {
        setEdgeValue(edge.getSource(), edge.getDest(), (E) edge.getWeight());
    }
    ...
    public boolean isEdge(int source, int dest) {
        return getEdgeValue(source, dest) != null;
    }
    ...
    /** Method to set the value of an edge
     ...
     @param wt - The weight
     */
    private void setEdgeValue(int source, int dest, E wt) {
     ...
     */
    private E getEdgeValue(int source, int dest) {
        if (isDirected() | source <= dest) {
    ...
    private class Iter
        ...
        private void advanceIndex() {
            ...
            while (index != getNumV()
                    && null == getEdgeValue(source, index));
        }
    }
